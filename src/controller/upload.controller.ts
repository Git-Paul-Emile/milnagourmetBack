...
class UploadController {
  ...
  private handleUpload(folder: string, successMessage: string) {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      uploadSingle(req, res, async (err) => {
        if (err) {
          if (err instanceof multer.MulterError && err.code === 'LIMIT_FILE_SIZE') {
            return res.status(StatusCodes.BAD_REQUEST).json(
              jsonResponse({ status: 'error', message: 'Le fichier est trop volumineux (max 5MB)' })
            );
          }
          return next(err);
        }
        ...
        if (!req.file) {
          return res.status(StatusCodes.BAD_REQUEST).json(
            jsonResponse({ status: 'error', message: 'Aucun fichier fourni' })
          );
        }
  ...
  deleteImage = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!publicId || !publicId.startsWith(`${CLOUDINARY_ROOT_FOLDER}/`)) {
        return res.status(StatusCodes.BAD_REQUEST).json(
          jsonResponse({ status: 'error', message: 'Identifiant d\'image (publicId) requis' })
        );
      }
      ...
      if (result.result !== 'ok' && result.result !== 'not found') {
        return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(
          jsonResponse({ status: 'error', message: 'Erreur lors de la suppression de l\'image' })
        );
      }
      if (result.result === 'not found') {
        return res.status(StatusCodes.NOT_FOUND).json(
          jsonResponse({ status: 'error', message: 'Image non trouvée' })
        );
      }

      res.status(StatusCodes.OK).json(
        jsonResponse({ status: 'success', message: 'Image supprimée avec succès' })
      );
    } catch (error) {
      next(error);
    }
  ...
  listImages = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const usedImages = await this.getUsedImagesList();
      const allImages: { value: string; label: string; isUsed: boolean; publicId: string }[] = [];
      ...
      do {
        ...
        for (const resource of response.resources as { public_id: string; secure_url: string }[]) {
