...
import avatarToastService from '../services/avatarToast.service.js';
...
class SiteController {
  ...
  // Mettre à jour l'avatar pour les toasts
  async updateAvatarToast(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const avatarToast = await avatarToastService.updateAvatarToast(image);

      res.status(StatusCodes.OK).json(
        jsonResponse({
          status: 'success',
          message: 'Avatar toast mis à jour avec succès',
          data: avatarToast
        })
      );
    } catch (error) {
  ...
  async getSocialMedia(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getContactSection(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async getCatalogSection(req: Request, res: Response, next: NextFunction) {
    try {
  ...
  async updateCatalogSection(req: Request, res: Response, next: NextFunction) {
    try {
      const {
        title,
        description,
  ...
  async getStoreHours(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      for (const [dayKey, dayData] of Object.entries(contactInfo.hours)) {
        ...
        const hourData = dayData as { open: string; close: string; closed: boolean };
  ...
  async updateStoreHours(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const hoursMap: { [key: string]: { open: string; close: string; closed: boolean } } = {};
      const dayMap: { [key: string]: string } = {
        'Lundi': 'monday',
        'Mardi': 'tuesday',
        'Mercredi': 'wednesday',
