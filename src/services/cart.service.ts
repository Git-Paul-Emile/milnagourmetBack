...
class CartService {
  ...
  async getUserCart(utilisateurId: number): Promise<CartWithItems | null> {
    try {
  ...
  async addToCart(utilisateurId: number, produitId: number, quantite: number): Promise<void> {
    try {
  ...
  async updateCartItem(utilisateurId: number, produitId: number, quantite: number): Promise<void> {
    try {
  ...
  async removeFromCart(utilisateurId: number, produitId: number): Promise<void> {
    try {
      const cart = await this.cartRepository.findByUserId(utilisateurId);
      if (!cart) {
        throw new Error('Panier non trouvé');
  ...
  // Vider le panier
  async clearCart(utilisateurId: number): Promise<void> {
    try {
      const cart = await this.cartRepository.findByUserId(utilisateurId);
      if (cart) {
  ...
  async addCustomCreation(utilisateurId: number, creationData: {
    tailleId: number;
    quantite: number;
    prix: number;
    fruits?: string[];
  ...
  async updateCustomCreation(utilisateurId: number, creationId: number, updateData: {
    quantite?: number;
    prix?: number;
    fruits?: string[];
    sauces?: string[];
  ...
  async removeCustomCreation(utilisateurId: number, creationId: number): Promise<void> {
    try {
          ...
          creationsPersonnalisees: cart.creations ? {
            create: cart.creations.map(creation => ({
              ...
              fruits: creation.fruits ? {
                create: JSON.parse(creation.fruits).map((fruitName: string) => ({
                  fruit: { connect: { nom: fruitName } }
                }))
              } : undefined,
              sauces: creation.sauces ? {
                create: JSON.parse(creation.sauces).map((sauceName: string) => ({
                  sauce: { connect: { nom: sauceName } }
                }))
              } : undefined,
              cereales: creation.cereales ? {
                create: JSON.parse(creation.cereales).map((cerealeName: string) => ({
