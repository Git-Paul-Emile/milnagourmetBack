...
class HeroService {
  async getHeroData() {
    try {
      ...
      return {
        ...
        features: hero.caracteristiques.map((feature: any) => ({
          title: feature.titre,
  ...
  async updateHeroData(data: { title?: string; subtitle?: string; badge?: string; banner?: string }) {
    try {
      const hero = await prisma.sectionHero.findFirst();
      ...
      return {
        ...
        features: updatedHero.caracteristiques.map((feature: any) => ({
          title: feature.titre,
          description: feature.description
        }))
      };
