    ...
    async getBranding() {
        try {
            ...
            if (activeTheme && activeTheme.name === 'Noël') {
                logo = '/src/assets/logos/noel-removebg-preview-1766180800922-611188349.png';
            }
            else {
                logo = '/src/assets/logos/milna-logo.png';
            }
            // Vérifier s'il y a un logo personnalisé dans la base de données
            const branding = await prisma.marque.findFirst();
            if (branding && branding.logo && !branding.logo.startsWith('/src/assets/logos/')) {
