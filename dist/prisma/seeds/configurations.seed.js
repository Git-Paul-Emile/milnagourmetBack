...
export async function seedConfigurations(prisma) {
    // Seed status configurations
    console.log('⚙️ Création des configurations de statuts...');
    const statusConfigs = [
        { statut: 'RECU', libelleFr: 'Reçue', couleurBg: 'bg-blue-100', couleurText: 'text-blue-800', icone: 'CheckCircle', ordre: 1 },
        { statut: 'EN_PREPARATION', libelleFr: 'En préparation', couleurBg: 'bg-orange-100', couleurText: 'text-orange-800', icone: 'ShoppingBag', ordre: 2 },
        { statut: 'LIVRAISON', libelleFr: 'En livraison', couleurBg: 'bg-green-100', couleurText: 'text-green-800', icone: 'Truck', ordre: 3 },
        { statut: 'LIVREE', libelleFr: 'Livrée', couleurBg: 'bg-purple-100', couleurText: 'text-purple-800', icone: 'CheckCircle', ordre: 4 }
    ];
    for (const config of statusConfigs) {
        await prisma.configurationStatut.upsert({
            where: { statut: config.statut },
            update: {
    ...
    for (const category of productCategories) {
        const translation = categoryTranslations.find(t => category.nom.toLowerCase().includes(t.code.toLowerCase()));
        if (translation) {
...
//# sourceMappingURL=configurations.seed.js.map